...
export default function ChatReports() {
  ...
  async function fetchReportes() {
    ...
    try {
      const { count } = await supabaseAdmin
        ...
        .in('tipo_contenido', ['texto', 'audio', 'imagen', 'video_chat'])
  ...
  const handleResolveReport = async () => {
    ...
    try {
      ...
      // 2. Si se seleccionó eliminar contenido y hay un mensaje con contenido multimedia
      if (resolutionForm.eliminarContenido && messageDetails?.url_contenido) {
        try {
          // Obtener el path del archivo desde la URL
          const urlParts = messageDetails.url_contenido.split('/');
          const filePath = `${messageDetails.emisor_id}/${urlParts[urlParts.length - 1]}`;
          ...
          // Actualizar el mensaje para eliminar la referencia al contenido
          const { error: updateError } = await supabaseAdmin
            .from('mensaje')
            .update({
              url_contenido: null,
              contenido: '[Contenido eliminado por moderador]'
            })
            .eq('id', messageDetails.id);
  ...
  const handleExpandReporte = async (reporteId: string) => {
    ...
    // Si el reporte es de un mensaje específico
    if (reporte.contenido_id) {
      try {
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    {messageDetails && (
                      <div className="space-y-4">
                        <div className="bg-gray-50 p-4 rounded-lg">
                          <h4 className="font-semibold text-lg mb-2">Contenido Reportado</h4>
                          {messageDetails.tipo_contenido === 'texto' && (
                            <p className="text-gray-700">{messageDetails.contenido}</p>
                          )}
                          {messageDetails.tipo_contenido === 'imagen' && messageDetails.url_contenido && (
                            <img 
                              src={messageDetails.url_contenido} 
                              alt="Contenido reportado" 
                              className="max-w-full rounded-lg"
                            />
                          )}
                          {messageDetails.tipo_contenido === 'audio' && messageDetails.url_contenido && (
                            <audio controls className="w-full">
                              <source src={messageDetails.url_contenido} type="audio/mpeg" />
                              Tu navegador no soporta el elemento de audio.
                            </audio>
                          )}
                          {messageDetails.tipo_contenido === 'video_chat' && messageDetails.url_contenido && (
                            <video controls className="w-full rounded-lg">
                              <source src={messageDetails.url_contenido} type="video/mp4" />
                              Tu navegador no soporta el elemento de video.
                            </video>
                          )}
                          <p className="mt-2 text-sm text-gray-500">
                            Enviado el: {new Date(messageDetails.fecha_envio).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    )}
